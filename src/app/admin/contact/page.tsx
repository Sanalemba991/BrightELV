...
import { FiMail, FiPhone, FiEye, FiTrash2, FiX, FiMessageSquare, FiCheck, FiClock, FiSearch } from 'react-icons/fi';
...
export default function ContactsPage() {
    ...
    const getStatusIcon = (status: string) => {
        switch (status) {
            ...
            case 'read': return <FiEye className="w-3 h-3" />;
            case 'replied': return <FiCheck className="w-3 h-3" />;
            default: return null;
        }
    };
                        ...
                        >
                            <div className="p-6 flex items-start gap-4">
                                <div className={`w-12 h-12 rounded-full flex items-center justify-center text-lg font-semibold ${
                                    contact.status === 'new' ? 'bg-yellow-500/20 text-yellow-400' : 'bg-orange-500/20 text-orange-400'
                                }`}>
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-3 mb-1">
                                        ...
                                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium border ${getStatusColor(contact.status)}`}>
                                            {getStatusIcon(contact.status)}
                                            {contact.status}
                                        </span>
                                    </div>
                                    <p className="text-gray-400 text-sm mb-2">{contact.subject}</p>
                                    <p className="text-gray-500 text-sm line-clamp-1">{contact.message}</p>
                        ...
                        >
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="flex items-center gap-4 p-4 rounded-xl bg-slate-700/30">
                                    <div className="w-16 h-16 rounded-full bg-purple-500/20 flex items-center justify-center text-2xl font-semibold text-purple-400">
                                        {selectedContact.name.charAt(0).toUpperCase()}
                                    </div>
                                    <div>
                                        <h3 className="text-xl font-semibold text-white">{selectedContact.name}</h3>
                                        <div className="flex items-center gap-3 text-gray-400 text-sm mt-1">
                                            <span className="flex items-center gap-1"><FiMail /> {selectedContact.email}</span>
                                            <span className="flex items-center gap-1"><FiPhone /> {selectedContact.phone}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                {/* Subject & Message */}
                                <div>
                                    <h4 className="text-lg font-semibold text-white mb-2">{selectedContact.subject}</h4>
                                ...
                                <div className="flex items-center justify-between pt-4 border-t border-white/10">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <select
                                            ...
                                            className={`px-4 py-2 rounded-lg border text-sm font-medium ${getStatusColor(selectedContact.status)}`}
                                        >
                                            <option value="new">New</option>
                                            <option value="read">Read</option>
                                            <option value="replied">Replied</option>
