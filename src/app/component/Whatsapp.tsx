...
const Whatsapp = () => {
  ...
  const handleWhatsApp = () => {
    const phoneNumber = '+971 50 881 3601';
    const message = 'Bright ELV Technology LLC! I would like to get in touch.';
    const whatsappUrl = `https://wa.me/${phoneNumber}?text=${encodeURIComponent(message)}`;
    window.open(whatsappUrl, '_blank');
  };

  const handleCall = () => {
    window.open('tel:+971 565022960', '_self');
  };

  const handleEmail = () => {
    const email = 'sales@brightelv.com';
    const subject = 'Inquiry from Website';
    const body = 'Hello Bright ELV Technology LLC Team,\n\nI would like to get in touch regarding...';
  ...
  return (
    <div className="fixed bottom-4 right-4 md:bottom-6 md:right-6 z-[9999] group">
      ...
      <div className="relative flex items-center">
        ...
        <button
          onClick={handleCall}
          className={`absolute flex items-center justify-center w-10 h-10 sm:w-11 sm:h-11 md:w-12 md:h-12 bg-gradient-to-br from-blue-900 to-blue-600 text-white rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 group/phone
            ${isOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}
          `}
          style={{
        ...
        <button
          ...
          title="Send Email"
        >
          <Mail size={16} className="sm:w-[18px] sm:h-[18px] md:w-[18px] md:h-[18px] text-blue-900 group-hover:text-blue-800" />
        ...
        <button
          ...
          className={`relative flex items-center justify-center w-12 h-12 sm:w-[52px] sm:h-[52px] md:w-14 md:h-14 rounded-full shadow-lg transition-all duration-500 hover:scale-105 ${
            isOpen 
              ? 'bg-gradient-to-br from-blue-900 to-blue-900 hover:from-blue-900 hover:to-blue-700 text-white' 
              : 'bg-gradient-to-br from-blue-900 to-blue-900 hover:from-blue-900 hover:to-blue-700 text-white animate-bounce'
          }`}
